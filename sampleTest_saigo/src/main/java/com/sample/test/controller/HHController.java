...
//		System.out.println(personList);
	...
	public @ResponseBody ArrayList<Request> selectComName(HttpSession session){
...
//		System.out.println("pickList 컨트롤러 : "+pickList);
	...
	@RequestMapping(value = "/updateMatchingStatus", method = RequestMethod.POST)
	public @ResponseBody int updateMatchingStatus(String personId, int status, HttpSession session) {
		
		String hhId=(String)session.getAttribute("hhId");
	
		int result=0;
		
		HashMap<String, Object> umsMap = new HashMap<String, Object>();
		
		umsMap.put("personId", personId);
		umsMap.put("status", status);
		umsMap.put("hhId", hhId);
		
		result=dao.updateMatchingStatus(umsMap);
		
		return result;
	}
	
	@RequestMapping(value = "/delCP", method = RequestMethod.POST)
	public @ResponseBody int delCP(int listSeq, HttpSession session) {
		
		int result=0;
		String hhId=(String)session.getAttribute("hhId");

		HashMap<String, Object> delCpMap = new HashMap<String, Object>();
		
		delCpMap.put("listSeq", listSeq);
		delCpMap.put("hhId",hhId);

		result=dao.delCP(delCpMap);
