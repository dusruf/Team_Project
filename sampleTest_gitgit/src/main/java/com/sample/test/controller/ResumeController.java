...
//	TotalResume totalResume = new TotalResume();
	...
	public String insertResume(TotalResume totalResume, HttpSession session, int updateFlag, int updateResumeSeq) {

		int deleteResult=0;
		
		if (updateFlag==1) {
			System.out.println("updateFlag : "+updateFlag);
			System.out.println("updateResume : "+updateResumeSeq);
			deleteResult=dao.deleteResume(updateResumeSeq);			
		}
		...
		System.out.println("basicInfo 등록 후 resumeSeq : "+resumeSeq);
	
		//education (학력)
		
		List<String> eduNum=totalResume.getSchoolName();
					
		if (eduNum!=null) {

			ArrayList<Education> eduList=new ArrayList<Education>();
			totalResume.setEducationList(eduList);
			
			System.out.println("for문 이전 totalResume : "+totalResume);
			
			for(int i=0;i<eduNum.size();i++) {
				
				//아쒸 스트링으로 넣어버렸다 어쩔 수 없어 한시가 급해
				String educationFlag=totalResume.getEducationFlag().get(i);
				String schoolName=totalResume.getSchoolName().get(i);
				String major=totalResume.getMajor().get(i);
				String diploma=totalResume.getDiploma().get(i);									
				String eduStartDate=totalResume.getEduStartDate().get(i);
				String eduEndDate=totalResume.getEduEndDate().get(i);
				
				Education education=new Education();
				
				education.setResumeSeq(resumeSeq);
				education.setEducationFlag(educationFlag);
				education.setSchoolName(schoolName);
				education.setMajor(major);
				education.setDiploma(diploma);
				education.setEduStartDate(eduStartDate);
				education.setEduEndDate(eduEndDate);		
				
				totalResume.getEducationList().add(education);
			...
			educationList=totalResume.getEducationList();
			
			System.out.println("insertEducationList : "+educationList);
			
			boolean flag = false;
			
			for (int i = 0; i < educationList.size(); i++) {
				int educationResult=dao.insertEducation(educationList.get(i));
				
				if(educationResult==1) flag=true;
				else flag=false;
			...
			System.out.println("education 속성 없음");
	...
	public String resumeUpdate(Model model, HttpSession session) {
		...
		ArrayList<Education> educationList=totalResume.getEducationList();
		System.out.println("컨트롤러 educationList 확인 : "+educationList);
...
//		session.setAttribute("totalResume", totalResume);
	...
	@RequestMapping(value="/readResume", method=RequestMethod.GET)
	public @ResponseBody int readResume(String personId){
